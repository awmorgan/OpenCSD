...
func makeMemRegionAccessCB(cbCtx *memRegionCallbackCtx) ocsd.MemAccessor {
	...
	return func(address ocsd.VAddr, _ ocsd.MemSpaceAcc, _ uint8, reqBytes uint32, byteBuffer []byte) uint32 {
...
func runSnapshotDecode(snapshotDir, sourceName string, packetOnly bool, opts etmv4DecodeOptions) ([]byte, error) {
	...
	sourceTree, ok := snapshot.SourceTree(resolvedSourceName, reader.Trace)
...
func resolveETMv4SourceName(requested string, trace *snapshot.Trace, devs map[string]*snapshot.Device) string {
	...
	for _, candidate := range candidates {
		...
		tree, ok := snapshot.SourceTree(candidate, trace)
