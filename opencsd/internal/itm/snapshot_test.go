...
func runITMSnapshotDecode(snapshotDir, sourceName string) ([]byte, error) {
	...
	for srcDevName := range sourceTree.SourceCoreAssoc {
		...
		if err := tree.AddWiredDecoder(traceID, ocsd.BuiltinDcdITM, ocsd.ProtocolITM, proc, dec, dec); err != nil {
	...
	if srcIsFrame {
		...
		if len(pending) > 0 {
			consumed, resp, _ := tree.TraceDataIn(ocsd.OpData, ocsd.TrcIndex(traceIndex), pending)
			if ocsd.DataRespIsFatal(resp) {
				return nil, fmt.Errorf("fatal datapath response at trace index %d", traceIndex)
			}
			if consumed > 0 {
