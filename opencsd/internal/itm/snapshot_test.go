...
func TestITMSnapshotsAgainstGolden(t *testing.T) {
	...
	testCases := []struct {
		...
		// itm-decode-test.ppl is generated by the standalone C++ itm-decode-test program
		// using synthetic (non-snapshot) trace data. After sanitisation it produces an
		// identical packet sequence to itm_only_raw, so it is omitted here to avoid a
		// phantom duplicate that masks rather than validates decoder behaviour.
	}
