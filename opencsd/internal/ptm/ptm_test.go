...
func mustNewPktDecode(tb testing.TB, config *Config) *PktDecode {
	tb.Helper()
...
func TestPTMTypedConstructors(t *testing.T) {
...
func TestDecoderAtomProcessing(t *testing.T) {
	...
	pkt2.Atom.Num = 1
	pkt2.Index = 1
	resp := ocsd.DataRespFromErr(dec.processPacket(pkt2))
	if ocsd.DataRespIsFatal(resp) {
...
func TestDecoderAtomMultiStep(t *testing.T) {
	...
	pkt.Atom.Num = 1
	pkt.Index = 2
	resp := ocsd.DataRespFromErr(dec.processPacket(pkt))
	if ocsd.DataRespIsFatal(resp) {
