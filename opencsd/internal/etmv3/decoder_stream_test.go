package etmv3

import (
	"opencsd/internal/idec"
	"opencsd/internal/ocsd"
	"testing"
)

func TestDecoderAtomProcessing(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)

	dec.Reset(0)

	pkt := &Packet{}
	pkt.ResetState()
	pkt.Type = PktISync
	pkt.Addr = 0x1000
	pkt.CurrISA = ocsd.ISAArm
	pkt.ISyncInfo.Reason = 1
	pkt.Context.UpdatedC = true
	pkt.Context.CtxtID = 0x42
	writeDecodedPacket(dec, 0, pkt)

	pkt2 := &Packet{}
	pkt2.ResetState()
	pkt2.Type = PktPHdr
	pkt2.Atom.EnBits = 0x1
	pkt2.Atom.Num = 1
	resp := ocsd.DataRespFromErr(writeDecodedPacket(dec, 1, pkt2))
	if ocsd.DataRespIsFatal(resp) {
		t.Errorf("Atom E failed: %v", resp)
	}

	pkt3 := &Packet{}
	pkt3.ResetState()
	pkt3.Type = PktPHdr
	pkt3.Atom.EnBits = 0x0
	pkt3.Atom.Num = 1
	writeDecodedPacket(dec, 2, pkt3)

	pkt4 := &Packet{}
	pkt4.ResetState()
	pkt4.Type = PktPHdr
	pkt4.Atom.EnBits = 0x5 // E, N, E
	pkt4.Atom.Num = 3
	writeDecodedPacket(dec, 3, pkt4)

	if len(drainDecodedElements(t, dec)) == 0 {
		t.Errorf("Expected trace elements from atom processing")
	}
}

func TestDecoderWPUpdate(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)
	dec.Reset(0)

	pkt := &Packet{}
	pkt.ResetState()
	pkt.Type = PktISync
	pkt.Addr = 0x2000
	pkt.CurrISA = ocsd.ISAArm
	pkt.ISyncInfo.Reason = 1
	writeDecodedPacket(dec, 0, pkt)

	// Since ETMv3 doesn't have PktWPointUpdate, let's test BranchAddress instead
	pkt2 := &Packet{}
	pkt2.ResetState()
	pkt2.Type = PktBranchAddress
	pkt2.Addr = 0x3000
	writeDecodedPacket(dec, 1, pkt2)

	if len(drainDecodedElements(t, dec)) == 0 {
		t.Errorf("Expected trace elements from WP update")
	}
}

func TestDecoderBranchWithException(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)
	dec.Reset(0)

	pkt := &Packet{}
	pkt.ResetState()
	pkt.Type = PktISync
	pkt.Addr = 0x1000
	pkt.CurrISA = ocsd.ISAArm
	writeDecodedPacket(dec, 0, pkt)

	pkt3 := &Packet{}
	pkt3.ResetState()
	pkt3.Type = PktBranchAddress
	pkt3.Exception.Present = true
	pkt3.Exception.Type = ocsd.ExcpFIQ // Or ArmV7Exception mapped
	pkt3.Exception.Number = 5
	pkt3.CurrISA = ocsd.ISAArm
	pkt3.Addr = 0x4000
	pkt3.CycleCount = 10
	writeDecodedPacket(dec, 2, pkt3)

	if len(drainDecodedElements(t, dec)) == 0 {
		t.Errorf("Expected trace elements")
	}
}

func TestDecoderMemNacc(t *testing.T) {
	config := &Config{}
	mem := &mockMemAcc{failAfter: 1, hitAfter: -1} // fail on 2nd read, never find branch
	dec := mustNewPktDecode(t, config, mem, idec.NewDecoder())

	dec.Reset(0)

	pkt := &Packet{}
	pkt.ResetState()
	pkt.Type = PktISync
	pkt.Addr = 0x1000
	pkt.CurrISA = ocsd.ISAArm
	writeDecodedPacket(dec, 0, pkt)

	pkt2 := &Packet{}
	pkt2.ResetState()
	pkt2.Type = PktPHdr
	pkt2.Atom.EnBits = 0x1
	pkt2.Atom.Num = 1
	writeDecodedPacket(dec, 1, pkt2)

	pkt3 := &Packet{}
	pkt3.ResetState()
	pkt3.Type = PktBranchAddress
	pkt3.Addr = 0x5000
	pkt3.CurrISA = ocsd.ISAArm
	writeDecodedPacket(dec, 2, pkt3)

	if len(drainDecodedElements(t, dec)) == 0 {
		t.Logf("No elements generated (memNacc path)")
	}
}

func TestDecoderContProcess(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)

	dec.Flush()
	dec.Reset(0)

	pkt := &Packet{}
	pkt.ResetState()
	pkt.Type = PktASync
	writeDecodedPacket(dec, 0, pkt)

	pkt.Type = PktContextID
	writeDecodedPacket(dec, 0, pkt)

	pkt.Type = PktCycleCount
	pkt.CycleCount = 100
	writeDecodedPacket(dec, 0, pkt)

	pkt.Type = PktTrigger
	writeDecodedPacket(dec, 0, pkt)

	_ = drainDecodedElements(t, dec)
}

func TestDecoderBranchVariations(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)
	dec.Reset(0)

	syncPkt := &Packet{}
	syncPkt.ResetState()
	syncPkt.Type = PktISync
	syncPkt.Addr = 0x8000
	writeDecodedPacket(dec, 0, syncPkt)

	// Branch missing CC
	pkt1 := &Packet{}
	pkt1.ResetState()
	pkt1.Type = PktBranchAddress
	pkt1.Addr = 0x2000
	pkt1.CurrISA = ocsd.ISAArm
	writeDecodedPacket(dec, 0, pkt1)

	pkt1.Exception.Present = true
	pkt1.ExceptionCancel = true
	pkt1.Context.UpdatedC = true
	pkt1.Context.CurrNS = true
	writeDecodedPacket(dec, 1, pkt1)

	pkt1.ExceptionCancel = false
	pkt1.Context.CurrNS = false
	writeDecodedPacket(dec, 2, pkt1)

	// Alt branch
	config.RegIDR = idrAltBranch | (4 << 4) // enable IsAltBranch
	pkt1.Exception.Present = true
	writeDecodedPacket(dec, 3, pkt1)

	// Unsync packet
	pkt2 := &Packet{}
	pkt2.ResetState()
	pkt2.Type = PktNotSync
	writeDecodedPacket(dec, 4, pkt2)
}

func TestDecoderPHeaderVariations(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)
	dec.Reset(0)

	pkt1 := &Packet{}
	pkt1.ResetState()
	pkt1.Type = PktISync
	pkt1.Addr = 0x1000
	pkt1.ISyncInfo.Reason = 2
	writeDecodedPacket(dec, 0, pkt1)

	pkt2 := &Packet{}
	pkt2.ResetState()
	pkt2.Type = PktPHdr
	pkt2.Atom.EnBits = 0x1
	pkt2.Atom.Num = 5
	pkt2.PHdrFmt = 1
	writeDecodedPacket(dec, 1, pkt2)

	// Fmt 2
	pkt2.PHdrFmt = 2
	pkt2.Atom.Num = 2
	writeDecodedPacket(dec, 2, pkt2)

	// Fmt 3
	pkt2.PHdrFmt = 3
	pkt2.CycleCount = 100
	writeDecodedPacket(dec, 3, pkt2)

	// Without Sync / unknown address
	dec.Reset(0)
	pkt2.PHdrFmt = 1
	writeDecodedPacket(dec, 4, pkt2)

	dec.Flush()
}

func TestDecoderAtomUsage(t *testing.T) {
	config := &Config{}
	mem := &mockMemAcc{failAfter: 10, hitAfter: 0, instrType: ocsd.InstrBr}
	dec := mustNewPktDecode(t, config, mem, idec.NewDecoder()) // conditional branch consumes atoms!

	dec.Reset(0)

	pkt1 := &Packet{}
	pkt1.ResetState()
	pkt1.Type = PktISync
	pkt1.Addr = 0x1000
	writeDecodedPacket(dec, 0, pkt1)

	// Send an atom packet E, N
	pkt2 := &Packet{}
	pkt2.ResetState()
	pkt2.Type = PktPHdr
	pkt2.Atom.Num = 2
	pkt2.Atom.EnBits = 1 // E, N
	writeDecodedPacket(dec, 1, pkt2)

	pkt3 := &Packet{}
	pkt3.ResetState()
	pkt3.Type = PktBranchAddress
	pkt3.Addr = 0x2000
	writeDecodedPacket(dec, 2, pkt3)

	elems := drainDecodedElements(t, dec)
	for _, e := range elems {
		t.Logf("Generated Element: %v", e.ElemType)
	}

	if len(elems) < 1 { // Allow at least 1 for now (IDSync)
		t.Errorf("Expected at least one element, got %d", len(elems))
	}
}

func TestDecoderAllPackets(t *testing.T) {
	config := &Config{}
	dec := setupDecFast(config)

	dec.Reset(0)

	syncPkt := &Packet{}
	syncPkt.ResetState()
	syncPkt.Type = PktISync
	syncPkt.Addr = 0x8000
	syncPkt.ISyncInfo.Reason = 0
	syncPkt.Context.UpdatedC = true
	syncPkt.Context.CtxtID = 42
	writeDecodedPacket(dec, 0, syncPkt)

	types := []PktType{
		PktCycleCount, PktTrigger, PktStoreFail,
		PktOOOData, PktOOOAddrPlc, PktNormData,
		PktDataSuppressed, PktValNotTraced, PktIgnore,
		PktVMID, PktExceptionEntry, PktExceptionExit, PktTimestamp,
	}

	for i, typ := range types {
		dec.Reset(0)

		writeDecodedPacket(dec, ocsd.TrcIndex(i), syncPkt)
		pkt := &Packet{}
		pkt.ResetState()
		pkt.Type = typ
		writeDecodedPacket(dec, ocsd.TrcIndex(i), pkt)
	}

	// Add missing error branches
	dec.Reset(0)
	writeDecodedPacket(dec, 0, syncPkt)
	errPkt := &Packet{}
	errPkt.ResetState()
	errPkt.Type = PktBadSequence
	writeDecodedPacket(dec, 0, errPkt)

	dec.Reset(0)
	writeDecodedPacket(dec, 0, syncPkt)
	errPkt.Type = PktReserved
	writeDecodedPacket(dec, 0, errPkt)

	dec.Flush()
}

func setupDecFast(config *Config) *PktDecode {
	dec, err := NewConfiguredPktDecode(0, config, &mockMemAcc{failAfter: -1}, idec.NewDecoder())
	if err != nil {
		panic(err)
	}
	return dec
}
