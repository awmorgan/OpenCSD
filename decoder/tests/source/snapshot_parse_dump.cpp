...
std::string Trim(const std::string &s) { return Util::Trim(s); }

std::string TrimQuotes(const std::string &s) { return Util::Trim(s, "\"'"); }

std::string NoneIfEmpty(const std::string &s) {
...
ParsedKeyValue SplitKeyValueRaw(const std::string &kv) {
  ...
  ParsedKeyValue out;
  out.raw_key = kv.substr(0, eq);
  out.raw_value = kv.substr(eq + 1);
  out.key = Trim(out.raw_key);
  out.value = Trim(out.raw_value);
  return out;
}
...
IniData ParseIniFile(const std::string &filePath) {
  ...
  while (std::getline(in, line)) {
    ...
    if (IsSectionHeader(line, sectionName)) {
      ...
      if (!have_section) {
        throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                        "Definition before section header in ini file: " +
                            filePath);
      }
      ParsedKeyValue kv = SplitKeyValueRaw(line);
      data[current_section].push_back(
          {kv.key, kv.value, kv.raw_key, kv.raw_value});
    }
  }
...
std::string NormalizePathForOutput(const std::string &path,
                                   bool strip_trailing) {
  std::string out = path;
...
void ParseRegMetadata(const std::string &raw_key, RegEntry &entry) {
  entry.raw_key = raw_key;
  std::string trimmed = Trim(raw_key);
  std::string::size_type open = trimmed.find('(');
  std::string::size_type close = trimmed.rfind(')');
  if (open == std::string::npos || close == std::string::npos ||
      close <= open) {
    entry.regname = Trim(trimmed);
    return;
  }

  entry.regname = Trim(trimmed.substr(0, open));
  std::string meta = trimmed.substr(open + 1, close - open - 1);
...
std::string NormalizeHexValue(const std::string &value, const std::string &size,
                              bool has_size) {
  ...
  bool size_ok = has_size && ParseUnsigned(size, size_bits) && size_bits > 0;
  if (IsHexString(trimmed)) {
    std::string digits = ToLowerHex(trimmed.substr(2));
    if (size_ok) {
      size_t width = static_cast<size_t>((size_bits + 3) / 4);
  ...
  if (size_ok) {
    uint64_t val = 0;
    if (ParseUnsigned(trimmed, val)) {
      std::ostringstream oss;
      size_t width = static_cast<size_t>((size_bits + 3) / 4);
      oss << "0x" << std::hex << std::nouppercase << std::setfill('0')
          << std::setw(static_cast<int>(width)) << val;
      return oss.str();
    }
  }
...
bool CaseInsensitiveEquals(const std::string &a, const std::string &b) {
  return Util::CaseInsensitiveEquals(a, b);
}

bool RegNameEquals(const std::string &a, const std::string &b) {
  return Util::CaseInsensitiveEquals(a, b);
}

bool HasRequiredRegs(const DeviceDump &device) {
  if (!CaseInsensitiveEquals(device.class_name, "trace_source"))
    return true;

  if (device.type_name.empty())
    return false;

  std::string type_name = device.type_name;
  std::string::size_type dot = type_name.find('.');
  if (dot != std::string::npos)
    type_name = type_name.substr(0, dot);

  auto has_reg = [&](const char *name) {
    for (size_t i = 0; i < device.regs.size(); ++i) {
      if (RegNameEquals(device.regs[i].regname, name))
        return true;
    }
    return false;
  };

  if (CaseInsensitiveEquals(type_name, "ETM4")) {
    return has_reg(ETMv4RegCfg) && has_reg(ETMv4RegIDR) &&
           has_reg(ETMv4RegIDR0) && has_reg(ETMv4RegIDR2);
  }
  if (CaseInsensitiveEquals(type_name, "ETE")) {
    return has_reg(ETMv4RegCfg) && has_reg(ETMv4RegIDR) &&
           has_reg(ETMv4RegIDR0) && has_reg(ETMv4RegIDR2);
  }
  if (CaseInsensitiveEquals(type_name, "ETM3")) {
    return has_reg(ETMv3PTMRegIDR) && has_reg(ETMv3PTMRegCR) &&
           has_reg(ETMv3PTMRegCCER) && has_reg(ETMv3PTMRegTraceIDR);
  }
  if (CaseInsensitiveEquals(type_name, "PTM1") ||
      CaseInsensitiveEquals(type_name, "PFT1")) {
    return has_reg(ETMv3PTMRegIDR) && has_reg(ETMv3PTMRegCR) &&
           has_reg(ETMv3PTMRegCCER) && has_reg(ETMv3PTMRegTraceIDR);
  }
  if (CaseInsensitiveEquals(type_name, "STM")) {
    return has_reg(STMRegTCSR);
  }
  if (CaseInsensitiveEquals(type_name, "ITM")) {
    return has_reg(ITMRegTCR);
  }
...
int RunSnapshotDump(int argc, char *argv[]) {
  ...
  if (ss_dir.empty() || output_file.empty()) {
    throw ocsdError(
        OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
        "Usage: snapshot_parse_dump -ss_dir <snapshot_dir> -o <output_file>");
  }

  if (IsAbsolutePath(ss_dir)) {
    throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                    "snapshot_dir must be relative: " + ss_dir);
  }
  ...
  std::vector<std::pair<std::string, std::string>> device_list;
  {
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
      std::string path = TrimQuotes(entries[i].value);
      if (IsAbsolutePath(path)) {
        throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                        "Device list contains absolute path: " + path);
      }
      device_list.push_back(std::make_pair(entries[i].key, path));
    }
  }
  ...
  if (snapshot_ini.find("trace") != snapshot_ini.end()) {
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
      if (entries[i].key == "metadata") {
        trace_metadata = TrimQuotes(entries[i].value);
        break;
      }
    }
  }
  ...
  if (!trace_metadata.empty()) {
    if (IsAbsolutePath(trace_metadata)) {
      throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                      "Trace metadata path must be relative: " +
                          trace_metadata);
    }
  ...
  for (size_t i = 0; i < device_list.size(); ++i) {
    ...
    if (device_ini.find("regs") != device_ini.end()) {
      ...
      for (size_t j = 0; j < entries.size(); ++j) {
        ...
        reg.order = j;
        ParseRegMetadata(entries[j].raw_key, reg);
        device.regs.push_back(reg);
      }
    }

    for (IniData::const_iterator it = device_ini.begin();
         it != device_ini.end(); ++it) {
      ...
      if (section_name.rfind("dump", 0) == 0) {
        ...
        for (size_t j = 0; j < entries.size(); ++j) {
          if (entries[j].key == "file") {
            dump.file =
                NormalizePathForOutput(TrimQuotes(entries[j].value), false);
            if (IsAbsolutePath(dump.file)) {
              throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                              "Dump file path must be relative: " + dump.file);
            }
            got_file = true;
        ...
        if (!got_address || !got_file) {
          throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                          "Dump section missing file or address: " +
                              device_ini_rel + "/" + section_name);
        }
    ...
    std::sort(device.regs.begin(), device.regs.end(),
              [](const RegEntry &a, const RegEntry &b) {
                ...
                if (a.has_id != b.has_id)
                  return !a.has_id;
                if (a.has_id) {
                  if (a.id_is_numeric && b.id_is_numeric &&
                      a.id_num != b.id_num)
  ...
  std::sort(
      ...
      [](const DeviceDump &a, const DeviceDump &b) { return a.name < b.name; });

  std::set<std::string> device_names;
  for (size_t i = 0; i < devices.size(); ++i) {
    if (!device_names.insert(devices[i].name).second) {
      throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                      "Duplicate device name: " + devices[i].name);
    }
  }
  ...
  if (!trace_metadata.empty()) {
    ...
    if (trace_ini.find("trace_buffers") == trace_ini.end()) {
      throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                      "Missing required [trace_buffers] section in " +
                          trace_metadata);
    }
    ...
    trace_buffer_ids.erase(
        std::unique(trace_buffer_ids.begin(), trace_buffer_ids.end()),
    ...
    for (size_t i = 0; i < trace_buffer_ids.size(); ++i) {
      ...
      for (size_t j = 0; j < entries.size(); ++j) {
        if (entries[j].key == "name") {
          ...
          for (size_t f = 0; f < buffer.files.size(); ++f) {
            buffer.files[f] =
                NormalizePathForOutput(TrimQuotes(buffer.files[f]), false);
            if (IsAbsolutePath(buffer.files[f])) {
              throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                              "Trace buffer file path must be relative: " +
                                  buffer.files[f]);
            }
          }
          got_file = true;
      ...
      if (!got_name || !got_file) {
        throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                        "Trace buffer section missing name or file: " +
                            buffer_id);
      }
    ...
    if (trace_ini.find("core_trace_sources") != trace_ini.end()) {
      ...
      for (size_t i = 0; i < entries.size(); ++i) {
        core_trace_sources.push_back(
            std::make_pair(entries[i].key, entries[i].value));
      }
    }
  ...
  WriteLine(out, "dump_format.version = 1");
  ...
  WriteLine(out, "snapshot.description = " + NoneIfEmpty(snapshot_description));
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    ...
    WriteLine(out, "class = " + NoneIfEmpty(device.class_name));
    WriteLine(out, "type = " + NoneIfEmpty(device.type_name));
    WriteLine(out, "location = " + NoneIfEmpty(device.location));
    ...
    for (size_t r = 0; r < device.regs.size(); ++r) {
      ...
      std::string id_value = reg.has_id ? reg.id : "<none>";
      std::string size_value = reg.has_size ? reg.size : "<none>";
      std::string value = NormalizeHexValue(reg.value, reg.size, reg.has_size);
      std::string line = "reg." + reg.regname + " = " + value +
                         " ; id=" + id_value + " size=" + size_value +
                         " raw_key=\"" + reg.raw_key + "\"";
      WriteLine(out, line);
    }
    ...
    for (size_t d = 0; d < device.dumps.size(); ++d) {
      ...
      WriteLine(out, "space = " + NoneIfEmpty(dump.space));
      WriteLine(out, "address = " + dump.address_str);
      WriteLine(out, "length = " + NoneIfEmpty(dump.length_str));
      WriteLine(out, "offset = " + NoneIfEmpty(dump.offset_str));
    }
  }
  ...
  if (!clusters.empty()) {
    ...
    for (size_t i = 0; i < clusters.size(); ++i) {
      std::vector<std::string> devices_in_cluster =
          SplitCommaList(clusters[i].second);
      std::ostringstream joined;
      for (size_t d = 0; d < devices_in_cluster.size(); ++d) {
        if (device_names.find(devices_in_cluster[d]) == device_names.end()) {
          throw ocsdError(OCSD_ERR_SEV_ERROR, OCSD_ERR_TEST_SNAPSHOT_PARSE,
                          "Cluster device name not found: " +
                              devices_in_cluster[d]);
        }
        if (d > 0)
          joined << ",";
  ...
  if (!trace_metadata.empty()) {
    WriteLine(out, "trace.metadata = " +
                       NormalizePathForOutput(trace_metadata, false));
    ...
    for (size_t i = 0; i < trace_buffers.size(); ++i) {
      ...
      WriteLine(out, "format = " + NoneIfEmpty(buf.format));
  ...
  size_t core_count = 0;
  size_t trace_source_count = 0;
  bool required_regs_present = true;
  for (size_t i = 0; i < devices.size(); ++i) {
    if (CaseInsensitiveEquals(devices[i].class_name, "core"))
      core_count++;
    if (CaseInsensitiveEquals(devices[i].class_name, "trace_source"))
      trace_source_count++;
    if (!HasRequiredRegs(devices[i]))
      required_regs_present = false;
  }

  WriteLine(out, "[[validation]]");
  WriteLine(out, "devices.total = " + std::to_string(devices.size()));
  WriteLine(out, "devices.cores = " + std::to_string(core_count));
  WriteLine(out,
            "devices.trace_sources = " + std::to_string(trace_source_count));
  WriteLine(out, "trace.buffers = " + std::to_string(trace_buffers.size()));
  WriteLine(out, "trace.core_trace_sources = " +
                     std::to_string(core_trace_sources.size()));
  WriteLine(out, std::string("trace.required_regs_present = ") +
                     (required_regs_present ? "true" : "false"));
